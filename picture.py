...
def savePictures(itemPagesurl):
    ...
    try:
        ...
        htmlpath = etree.HTML(htmldata)
        print(itemPagesurl)
        pages = htmlpath.xpath('//div[@class="items clearfix"]/div/a/div/img/@src')
        names = htmlpath.xpath('//div[@class="items clearfix"]/div/a/div/img/@alt')
    except Exception:
        pass
    for i in range(len(pages) ):
        print(pages[i])
        print(g_dir+'/'+names[i]+'_'+g_pageNum+'_'+bytes(i))
        ...
        try:
            ...
            binfile = open(g_dir+'/%s.jpg' % ( names[i]+'_'+g_pageNum+'_'+bytes(i) ) , "wb")
...
if __name__ == "__main__":
    ...
    cmd = 'mkdir -p '+g_dir
    system(cmd)
    g_pageNum=sys.argv[2]
    savePictures(sys.argv[3])
"""
fl=open('list.txt', 'w')
